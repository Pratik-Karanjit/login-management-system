...
const DeleteAccount = () => {
  ...
  let deleteAdmin = async () => {
    try {
      await axios({
        url: `http://localhost:8000/users/delete?token=${getLoginInfo()?.token}`,         //sent login token in url itself so that backend's isAuthenticatedForEmail middleware can get the token from query
        method: "delete",
      });
