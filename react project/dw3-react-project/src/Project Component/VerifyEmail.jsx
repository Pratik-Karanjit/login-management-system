...
import '../Project CSS/verifyEmailPage.css';
...
const VerifyEmailPage = () => {
  ...
  useEffect(() => {
    ...
    if (token) {                                                           //if there is token, send the token to verifyEmail 
      verifyEmail(token);     
    } else {
  ...
  const verifyEmail = async (token) => {                                   //token got from line 14
    try {
      const response = await axios.post('http://localhost:8000/users/verify-email', { isVerify: true }, {     //POST request to the server to verify the email
      ...
      if (data.success && !data.isVerify) {           //if data is success and isVerify is false, email verification message is sent else failed.
        setVerificationStatus('Email verification successful!');
      } else {
        setVerificationStatus(`Email verification failed: ${data.message}`);
      }
    } catch (error) {                //If any other error 
      console.error(error);
